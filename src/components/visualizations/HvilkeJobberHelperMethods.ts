...
export function loadTSV(
  ...
  const tplMappings: { [id: string]: string[] } = {
    ...
    over_under_40: [
      "antall personer under 40 år",
      "antall personer over 40 år",
    ],
    kandidater_13: [
      "Nyutdanna",
  ...
  d3.tsv("https://groven.no/utdno/yustat/data/" + id + ".tsv").then(function(
    ...
    const mean = d3.mean(data, (d: any) => d.sum);
    if (mean) {
      const far = d3.max(data, (d: any) => Math.abs(d.sum - mean));
      if (far) {
    ...
    setTimeout(function() {
      showTop10.style("display", "inline").attr("transform", function() {
        if (this) {
      ...
      showTop10
        ...
        .attr("width", function() {
          if (this) {
            var thisElement = this as any;
            const bbox = thisElement.parentElement.getBBox();
    ...
    function setMapping(tpl: string) {
      types = ["series-0", "series-1", "series-2"];
      moveBars(500);

      mapping = {};
      tplMappings[tpl].forEach((v, i) => {
        mapping[`series-${i}`] = v;
      });
    ...
    function createLegend() {
      ...
      colorControler
        ...
        .text((d: any) =>
          mapping[d]
            ? mapping[d].charAt(0).toUpperCase() +
              mapping[d].substring(
                1
              ) /*.replace(/(ukjent|uoppgitt)/, 'prikket')*/
            : ""
        );
    }
    ...
    function resize() {
      ...
      size.height = data.length * step + margin.top + margin.bottom;
    ...
    function moveBars(duration = 0, delay = 0, key = null) {
      // rows.attr("display", (d, i) => { return (yScale.domain().indexOf(d.name) < 10 ? 'inline' : 'none'); });
      ...
      if (!showAll) {
        ...
        svg.select(".show_top10").attr("transform", function() {
          if (this) {
            var thisElement = this as any;
            const bbox = thisElement.getBBox();
            const x = size.width - margin.right - bbox.width - 12;
